...
---
# P6F Device / Geography Performance Rules

The P6F Device / Geography Performance Mart applies the existing governed KPI contracts at the approved analytical grain:

`session_date + device_category + country`

The mart must not introduce alternative definitions of existing governed KPIs.

## Segmentation Source

`device_category` and `country` are governed session-level analytical attributes sourced from `fct_sessions`.

Both attributes are assigned to the governed session before business-mart aggregation.

The mart must not reconstruct device or geography attributes from raw GA4 events.

## Session Date

All P6F measures use the governed:

`fct_sessions.session_date`

as their reporting date.

Raw GA4 `event_date` must not be used to independently construct the P6F daily grain.

This preserves one governed session population when a session spans more than one raw event date.

## Device Category

`device_category` represents the device category associated with the governed session.

Observed source profiling identified three valid device categories and no sessions with an invalid device category in the current observation window.

For defensive downstream handling, a future null, blank, or `(not set)` device category must be represented as `Unknown` rather than excluded from the mart population.

## Country

`country` represents the country associated with the governed session.

Missing, blank, or `(not set)` country values must be normalized to:

`Unknown`

within the P6F business mart.

Unknown-country sessions must remain in the mart population so that segment-level totals reconcile to the governed session population.

## Session Count

`session_count` uses the existing governed Session Count contract.

At the P6F grain:

`session_count = COUNT(*)`

over `fct_sessions` grouped by:

`session_date + device_category + governed_country`

## Purchasing Session Count

`purchasing_session_count` uses the existing governed Purchasing Session Count contract.

At the P6F grain:

`purchasing_session_count = COUNTIF(has_purchase)`

A session contributes at most once regardless of the number of transactions generated by that session.

## Session Conversion Rate

`conversion_rate` uses the existing governed Session Conversion Rate contract.

The formula remains:

`purchasing_session_count / session_count`

Both numerator and denominator must belong to the same device-category, country, and governed session-date population.

## Transaction Count

For P6F, `transaction_count` is a session-attributed commercial measure.

The mart must aggregate the governed `transaction_count` already associated with each row of `fct_sessions`.

Transactions are therefore attributed to:

- the originating session's `session_date`
- the originating session's `device_category`
- the originating session's governed country

P6F `transaction_count` must not be independently aggregated by `fct_transactions.transaction_date`.

## Purchase Revenue

For P6F, `purchase_revenue` is a session-attributed commercial measure.

The mart must aggregate the governed `purchase_revenue` associated with each row of `fct_sessions`.

Purchase revenue is therefore attributed to:

- the originating session's `session_date`
- the originating session's `device_category`
- the originating session's governed country

P6F purchase revenue must not be independently aggregated by transaction date.

## Revenue per Session

`revenue_per_session` uses the existing governed Revenue per Session contract.

At the P6F grain:

`revenue_per_session = session-attributed purchase_revenue / session_count`

The numerator and denominator must represent the same governed device-category, country, and session-date population.

## Average Order Value

`average_order_value` is not included in the current P6F mart contract.

The governed general AOV definition remains transaction-based and uses transaction-date semantics when reported over time.

Introducing AOV into a session-segment mart would require an explicit session-attributed AOV contract and business requirement.

It must not be introduced implicitly by dividing P6F session-attributed purchase revenue by transaction count without an approved design change.

## Reaggregation

The following P6F measures are additive across compatible mutually exclusive segment rows:
...
The following P6F ratios are non-additive:

- `conversion_rate`
- `revenue_per_session`

When the reporting grain changes, ratios must be recalculated from aggregated governed numerators and denominators rather than summed or averaged from mart rows.
...
- session-attributed `transaction_count` must reconcile to the governed transaction population
- session-attributed `purchase_revenue` must reconcile to governed purchase revenue

Unknown-country sessions must remain included in these reconciliations.
